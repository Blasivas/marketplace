...
export function EditarProduto() {
  ...
  const { handleSubmit, register, formState, setValue } = useForm<EditarProdutoForm>({resolver: zodResolver(EditarProdutoForm)})
  ...
  const { mutateAsync: edit } = useMutation({
    mutationFn: ({ id, body }: { id: string, body: EditProductBody }) => editProduct(id, body),
  })
        ...
        <div className="flex flex-col bg-white rounded-[20px] p-8 w-[591px] gap-8">
          <div className="flex justify-between">
            ...
            <span className="flex label-sm text-white bg-blue-dark rounded-2xl justify-center items-center px-2 py-1">
              {data?.product?.status ? ProductStatus[data.product.status as keyof typeof ProductStatus] : ""}
            </span>
          </div>

          <form onSubmit={handleSubmit(handleEditarProduto)} className="flex flex-col gap-5 ">
            <div className="flex gap-5">
              ...
              className="flex flex-col label-md text-gray-300 focus-within:text-orange-base w-7/10 border-b-gray-200 border-b-2 ">TÍTULO      
                <input id='title' type='text' {...register('title')} className="body-md text-gray-400 w-full h-5 my-3.5" />
              </label>
              ...
              <label htmlFor='value' className="flex flex-col label-md text-gray-300 focus-within:text-orange-base border-b-gray-200 border-b-2">VALOR            
                  <div className="flex items-center gap-1">
                    ...
                    <input id='value' type='number' step='0.01' {...register('value', { valueAsNumber: true })} className="body-md text-gray-400 w-full h-5 my-3.5" />                
                  </div>
              </label>
            </div>
            ...
            <label htmlFor='description' className="flex flex-col label-md text-gray-300 focus-within:text-orange-base border-b-gray-200 border-b-2">DESCRIÇÃO              
                <textarea id='description' rows={5} {...register('description')} className="body-md text-gray-400 w-full my-3.5 resize-none" />              
            </label>
